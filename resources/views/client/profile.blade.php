@extends('layouts.client')
@section('content')

    <div class="header" data-aos="fade-right" data-aos-duration="1000" style="background-image: url('{{asset('assets/client/building2.jpg')}}')">

      <div  class="p-4 text-center bg-image"  style="filter: brightness(60%);">
        <img src="" alt="">
        </div>
      <div class="d-flex justify-content-center align-items-center" >
        <div class="banner-title text-center">
          <h1 class="mb-3 text-light" data-aos="fade-right" data-aos-duration="1000"><span>Architect's Profile</span></h1>
          <p class="mb-3 text-light text-center" data-aos="fade-right" data-aos-duration="1000" style="font-size:20px;">Get to Know our Finest Architects</p>
          
        </div>
      </div>
    </div>
    <br>
  

   <!-- ======= About Section ======= -->
   <section class="about section-padding" data-aos="fade-right" data-aos-duration="1000" id="about">
		<div class="container">
			<div class="row" data-aos="fade-up" data-aos-delay="750">
				<div class="col-lg-4 col-md-12 col-12">
					<div class="about-img" data-aos="fade-up" data-aos-delay="750"><img class="img-fluid" src="{{asset('assets/client/ar2.jpg')}}" style="height: 400px; width:700px; align-items:justify;"></div>
				</div>
				<div class="col-lg-8 col-md-12 col-12 ps-lg-5 mt-md-5">
					<div class="about-text">
						<h2 style="color:#0d6efd;" class="animate__animated animate__fadeInLeft animate__delay-0.7s">Arch. Christopher G. Bana UAP</h2>
						<h3 style="font-size:20px; color:#ffcc00;">Co-Founder of Bana & Bana Architects</h3>
						<p class="animate__animated animate__fadeInLeft animate__delay-0.7s" style="text-align:justify;" data-aos="fade-up" data-aos-delay="750">  Arch. Christopher G. Bana is a product of Technological Institute of the Philippines graduated Bachelor of Science in Architecture in October 1992.
                He is a highly skilled and dedicated professional with 20 years of experience and has a strong background in SIte Supervising, Planning, & Technical Drawing Management.
                Highly commited to quality and teamwork. He provides technical skills and expertise in various architectural design of engineering structures. Prepare work plans and communicate
                to project managers to determine the scope of work. </p>
                
                <div class="row gy-4 mt-3">
              <div class="col-md-6 icon-box" data-aos="fade-up" data-aos-delay="750">
                <i class="bx bx-receipt"></i>
                <h4><a href="#">Site Engineer</a></h4>
                <p>Penny Steel Haus during 1993-1997. He also became a Site Engineer for Site Tech Electromechanical in Dubai, UEA in 2008-2010. </p>
              </div>
              <div class="col-md-6 icon-box" data-aos="fade-up" data-aos-delay="750">
                <i class="bx bx-cube-alt"></i>
                <h4><a href="#">Site Supervisor</a></h4>
                <p>Been a Site Supervisor while staying in Dubai starting from 2008-2011 for a Big Architectural Firm in the country that builds infrastructures.</p>
              </div>
              <div class="col-md-6 icon-box" data-aos="fade-up" data-aos-delay="750">
                <i class="bx bx-images"></i>
                <h4><a href="#">MEP Site Engineer</a></h4>
                <p>Became a Site Engineer and in charge in approving project designs and assigning engineers.</p>
              </div>
              <div class="col-md-6 icon-box" data-aos="fade-up" data-aos-delay="750">
                <i class="bx bx-shield"></i>
                <h4><a href="#">Co-Founder of Bana & Bana Architects</a></h4>
                <p>Co-Founder of Bana & Bana Architects since 2011 up to the present day</p>
              </div>
            </div>
          </div>
                
					</div>
				</div>
			</div>
		</div>

    
<br><br>
    <section class="about section-padding" id="about">
		<div class="container"data-aos="fade-up" data-aos-delay="750">
			<div class="row" data-aos="fade-up" data-aos-delay="750">
				<div class="col-lg-4 col-md-12 col-12">
					<div class="about-img "><img class="img-fluid" src="{{asset('assets/client/ar1.jpg')}}" style="height: 400px; width:700px; align-items:justify;"></div>
			
        </div>
				<div class="col-lg-8 col-md-12 col-12 ps-lg-5 mt-md-5">
					<div class="about-text">
						<h2 style="color:#0d6efd;" data-aos="fade-up" data-aos-delay="750">Arch. Michael G. Bana UAP</h2>
						<h3 style="font-size:20px; color:#ffcc00;" data-aos="fade-up" data-aos-delay="750">Co-Founder of Bana & Bana Architects</h3>
						<p style="text-align:justify;" data-aos="fade-up" data-aos-delay="750">  Arch. Michael G. Bana is a product of Technological Institute of the Philippines graduated Bachelor of Science in Architecture in October 1992.
                He is a highly skilled and dedicated professional with 20 years of experience and has a strong background in SIte Supervising, Planning, & Technical Drawing Management.
                Highly commited to quality and teamwork. He provides technical skills and expertise in various architectural design of engineering structures. Prepare work plans and communicate
                to project managers to determine the scope of work. </p>
                
                <div class="row gy-4 mt-3">
              <div class="col-md-6 icon-box" data-aos="fade-up" data-aos-delay="750">
                <i class="bx bx-receipt"></i>
                <h4><a href="#">Site Engineer</a></h4>
                <p>Penny Steel Haus during 1993-1997. He also became a Site Engineer for Site Tech Electromechanical in Dubai, UEA in 2008-2010. </p>
              </div>
              <div class="col-md-6 icon-box" data-aos="fade-up" data-aos-delay="750">
                <i class="bx bx-cube-alt"></i>
                <h4><a href="#">Site Supervisor</a></h4>
                <p>Been a Site Supervisor while staying in Dubai starting from 2008-2011 for a Big Architectural Firm in the country that builds infrastructures.</p>
              </div>
              <div class="col-md-6 icon-box" data-aos="fade-up" data-aos-delay="750">
                <i class="bx bx-images"></i>
                <h4><a href="#">MEP Site Engineer</a></h4>
                <p>Became a Site Engineer and in charge in approving project designs and assigning engineers.</p>
              </div>
              <div class="col-md-6 icon-box" data-aos="fade-up" data-aos-delay="750">
                <i class="bx bx-shield"></i>
                <h4><a href="#">Co-Founder of Bana & Bana Architects</a></h4>
                <p>Co-Founder of Bana & Bana Architects since 2011 up to the present day</p>
              </div>
            </div>
          </div>
               
					</div>
				</div>
			</div>
		</div>
    <br>
	</section><!-- about section Ends -->

  <br>

  <script>
  AOS.init();
</script>
@endsection